...
if st.button("🚀 Run Algo"):
    ...
    train_df = ml_result["training_df"]
    st.dataframe(train_df.tail(20), use_container_width=True)

    st.markdown(f"📦 **ML Dataset Size:** `{len(train_df)}` rows")
    st.markdown(
        "📌 **Target:** `1` = Next day price goes UP, `0` = price DOWN or no movement"
    )

    # === Dataset Split Info ===
    train_size = int(len(train_df) * 0.8)
    test_size = len(train_df) - train_size
    st.markdown(f"✅ **Train Size**: `{train_size}` | **Test Size**: `{test_size}`")

    # === EDA Plots ===
    st.subheader("📊 ML Dataset EDA")
    st.plotly_chart(
        px.pie(train_df, names="Target", title="Target Distribution (1=UP, 0=DOWN)"),
        use_container_width=True,
    )
    st.plotly_chart(
        px.histogram(
            train_df, x="RSI", color="Target", nbins=30, title="RSI Histogram"
        ),
        use_container_width=True,
    )
    st.plotly_chart(
        px.scatter(
            train_df, x="MACD", y="MACD_Signal", color="Target", title="MACD vs Signal"
        ),
        use_container_width=True,
    )
